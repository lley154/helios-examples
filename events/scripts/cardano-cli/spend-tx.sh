...
then
    ...
    echo "Usage: process-tx.sh [devnet|preview|preprod|mainnet] txHash txIdx"
    exit 1
fi

# Set command line arg variables
ENV=$1
txHash=$2
txIdx=$3
event_utxo=$txHash#$txIdx
...
# Step 2: Get the donation smart contract utxo
cardano-cli query utxo --tx-in  $event_utxo $network --out-file $WORK/validator-utxo.json

order_datum_in=$(jq -r '."'$event_utxo'".inlineDatum ' $WORK/validator-utxo.json)
...
# Step 4: Update shopify that the order is paid in full 
